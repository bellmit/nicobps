	...
	function ($scope, $http, $timeout, commonInitService) {
		...
		$scope.viewDetails = (applicant) => {
			...
			$scope.updateStakeholder = (applicant, process) => {
				let data = "usercode=" + applicant.usercode+"&toprocesscode="+process.toprocesscode+"&remarks="+process.flowname;
			...
			$scope.listNextProcess=(processcode)=>{
				$http.post("./listNextProcess.htm?",{"currentprocesscode":processcode}).success(
