...
app.controller('applicationController', function($scope) {
	...
	$scope.resetDetails = function() {
		...
		jQuery("#userpasswordconfirm").val('');
		jQuery("#percent").html("");
		jQuery("#results").html("");
		jQuery("#colorbar").css('width', 0);
		jQuery("#copyAddress").prop("checked", false);
	...
	$scope.copyAddress = function(status) {
		if (status) {
			...
			jQuery(".peradd").find("input,select").attr("disabled", "disabled");		
			...
			jQuery(".peradd").find("input,select").attr("disabled",false);		
...
function submitSignupDetails(signupDetails) {
	var flag = false;
	ConfirmBox(" Click Yes to register. Click No if you want to review your entries", (response) => {
