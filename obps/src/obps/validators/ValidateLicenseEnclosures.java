	...
	public void validate(Object o, Errors errors) {
		...
		if (licenseesenclosures.getAppenclosures() != null && licenseesenclosures.getAppenclosures().size() > 0) {
			if (environment.getProperty("captcha_enabled").equals("true")) {
				if (licenseesenclosures.getSessioncaptcha() != null && licenseesenclosures.getUserresponsecaptcha()
						.equalsIgnoreCase(licenseesenclosures.getSessioncaptcha())) {
					errorFlag = false;
				} else {
					errorFlag = true;
				}
		...
		for (AppEnclosures U : licenseesenclosures.getAppenclosures()) {
			if (U.getFileContent() != null) {
				...
				if (Patterns.PatternCompileMatche(Patterns.PATTERN_DOCUMENT_CONTANT, contenttype)
						&& UtilFile.isValidFile(commonsMultipartFile.getBytes(), Patterns.PATTERN_DOCUMENT_CONTANT)) {
